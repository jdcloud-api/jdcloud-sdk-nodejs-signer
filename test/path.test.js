...
describe('path',function () {
  ...
  it('pathName 包含多个/',function () {
    let url='http://test.jdcloud-api.com////v1/regions////cn-north-1//instances/'
    let ctx=new ContextV1(url,'POST',null,'test')

    assert.ok(ctx.pathName)
  })
  ...
  it('路径已经编码',function () {
    ...
    let ctx=new ContextV1(url,'POST',null,'test')
    console.log(ctx.pathName)
    assert(ctx.pathName ==='/v1/regions/cn-north-1/instances/ /`!@#$%^&*()=+/0123456789/[]\\\\;\',<>?:\\"{}|/abcdefghijklmnopqrstuvwxyz/ABCDEFGHIJKLMNOPQRSTUVWXYZ/-_.~:GET')
  })
})
